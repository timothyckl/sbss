...
class SimilarityStratifiedSplit:
    ...
    def __init__(
        self, n_splits: int, dist_func: Callable[[NDArray], NDArray], shuffle: bool = False
    ) -> None:
    ...
    def _validate(self, class_counts: NDArray, min_samples_per_class: int) -> None:
        ...
        if np.any(self.n_splits > class_counts):
            raise ValueError(
                "Number of folds cannot be greater than the number of members in each class."
            )

        if self.n_splits > min_samples_per_class:
            warnings.warn(
                f"The least populated class in labels has only {min_samples_per_class} members, "
    ...
    def split(
        self, X: NDArray, y: NDArray = None, groups=None
